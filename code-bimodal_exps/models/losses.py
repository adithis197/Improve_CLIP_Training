...
class GlobalContrastiveLoss(nn.Module):
    def __init__(self, temperature=0.07, reduction='mean'):
        ...
        Args:
            ...
            reduction: 'mean' or 'sum' for the final loss aggregation.
        """
    ...
    def forward(self, image_features, text_features):
        ...
        # Positive labels (diagonal)
        labels = torch.arange(logits.size(0)).to(image_features.device)

        # Debias the negative logits by subtracting their mean
        logits_debiased = logits - logits.mean(dim=1, keepdim=True)
...
class Modified_CLIP_Loss(nn.Module):
    def __init__(self, world_size=8, temperature=0.01, personalized_tau=False, image_tau=None, text_tau=None, pos_weight=1.0):
        super(Modified_CLIP_Loss, self).__init__()
        self.world_size = world_size
        self.temperature = temperature
        self.personalized_tau = personalized_tau  # if true, then temperatures are learnable
        self.image_tau = image_tau
        self.text_tau = text_tau
        self.pos_weight = pos_weight  # New: Positive pair weighting factor
    ...
    def forward(self, image_features, text_features, image_idx=None, text_idx=None):
        ...
        if self.personalized_tau:
            # Use learnable temperatures for personalized scaling
            image_temp = self.image_tau[image_idx]
            text_temp = self.text_tau[text_idx]
            pos_sim = sim / text_temp  # Positive scaling
            neg_sim = sim.t() / image_temp  # Negative scaling
        else:
            # Uniform temperature scaling
            pos_sim = sim / self.temperature
            neg_sim = sim.t() / self.temperature

        # Compute positive pair weighting
        pos_loss = F.cross_entropy(pos_sim, labels)
        neg_loss = F.cross_entropy(neg_sim, labels)
        total_loss = (self.pos_weight * pos_loss + neg_loss) / (1 + self.pos_weight)
