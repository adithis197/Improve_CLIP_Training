...
from models.losses import CLIP_Loss, CyCLIP_Loss, SogCLR_Loss, VICReg_Loss, Modified_CLIP_Loss
from models.losses import iSogCLR_New_v2_Loss, iSogCLR_New_v1_Loss, onlineCLR_Loss, iSogCLR_New_Loss
from models.losses import GlobalContrastiveLoss
...
class CLIP(nn.Module):
    def __init__(self,               
        ...
        # Handle loading ResNet50 model
        if image_encoder.endswith('.pth'):
            print(f"Loading ResNet50 offline weights from {image_encoder}")
            self.visual_encoder = timm.create_model('resnet50', pretrained=False)  # No pretrained weights
            state_dict = torch.load(image_encoder, map_location=torch.device('cpu'))  # Load offline weights
            self.visual_encoder.load_state_dict(state_dict)
        else:
            print(f"Using online pretrained ResNet50 model: {image_encoder}")
        ...
        elif self.ita_type == 'global_contrastive':
            self.criterion = GlobalContrastiveLoss()
        ...
        elif self.ita_type == 'modified_clip':
            if personalized_tau:  # For learnable temperatures
                image_tau = torch.ones(dataset_size).cuda() * self.temp
                text_tau = torch.ones(dataset_size).cuda() * self.temp
                self.criterion = Modified_CLIP_Loss(
                    temperature=self.temp,
                    world_size=world_size,
                    pos_weight=1.5,  # Example weight for positive pairs
                    personalized_tau=True,
                    image_tau=image_tau,
                    text_tau=text_tau
                    )
            else:  # For uniform temperature
                self.criterion = Modified_CLIP_Loss(
                temperature=self.temp,
                world_size=world_size,
                pos_weight=1.5,
    ...
    def forward(self, image, text, idx, text_idx, epoch, max_epoch):
        ...
        if self.ita_type in ['clip', 'cyclip', 'modified_clip']:
